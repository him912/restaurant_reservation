...
import { X, Mail, Lock, User, Eye, EyeOff, ShieldAlert, Sparkles, Building, ArrowRight, RefreshCw, KeyRound } from 'lucide-react';
        ...
        >
          ...
          <div className="p-6 pb-4 border-b border-slate-850 flex items-center justify-between bg-gradient-to-r from-indigo-950/40 to-slate-900">
            <div className="flex items-center gap-2">
              ...
              <div>
                <span className="block text-sm font-bold tracking-tight text-white leading-tight">DineFlow Portal</span>
                <span className="text-[9px] text-slate-400 font-mono tracking-widest uppercase">Elegance Verified</span>
              </div>
            </div>
          ...
          <div className="p-6 md:p-8">
            ...
            <div className="mb-6 text-center">
              ...
              <p className="text-slate-450 text-xs mt-1 leading-relaxed">
                {authModalTab === 'login' && 'Unlock instant dining slots and track active seating covers.'}
                {authModalTab === 'signup' && 'Gain custom VIP dining point tallies and critic status levels.'}
                {authModalTab === 'forgot' && 'Authenticate identity to reset system profile credentials.'}
              </p>
            </div>
            ...
            {authModalTab === 'forgot' && isResetSent ? (
              ...
              >
                ...
                <div className="space-y-1">
                  <span className="text-emerald-400 font-bold text-xs uppercase tracking-widest block">Recovery Dispatched</span>
                  <p className="text-slate-300 text-xs leading-relaxed max-w-sm mx-auto">
                    We've simulated sending a recovery link to <span className="font-bold text-white break-all">{email}</span>. Use the security credentials below:
                  </p>
                </div>

                <div className="bg-slate-950/80 border border-slate-800 rounded-2xl p-4 max-w-xs mx-auto">
                  <span className="block text-[9px] text-slate-500 font-mono tracking-widest uppercase">Secret OTP Code</span>
                  <span className="block text-2xl font-black text-indigo-400 tracking-widest mt-1 font-mono">{resetCode}</span>
                  <span className="block text-[9px] text-slate-400 mt-2">Default Password fallback matches <strong className="text-slate-205">password</strong></span>
                </div>
              ...
              <form onSubmit={handleFormSubmit} className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder={authModalTab === 'login' ? '••••••••' : 'Password (min. 6 characters)'}
                        value={password}
                ...
                {authModalTab === 'signup' && (
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-3" id="role-select-cards-grid">
                      <button
                        type="button"
                      ...
                      >
                        <User size={16} className={role === 'customer' ? 'text-indigo-400' : 'text-slate-500'} />
                        <div className="mt-3">
                          <span className="block text-xs font-bold leading-none">Dine Client</span>
                          <span className="block text-[9px] text-slate-450 mt-1">Acquire reserve slots</span>
                        </div>
                      </button>
                      ...
                      >
                        <Building size={16} className={role === 'owner' ? 'text-indigo-400' : 'text-slate-500'} />
                        <div className="mt-3">
                          <span className="block text-xs font-bold leading-none">Venue Vendor</span>
                          <span className="block text-[9px] text-slate-450 mt-1">Manage seat tables</span>
                        </div>
                      </button>
                    </div>
                  </div>
                    ...
                    {isSubmitting ? (
                      ...
                      <>
                        <span>
                          ...
                          {authModalTab === 'forgot' && 'REQUEST RESET INSTANTLY'}
                        </span>
                        <ArrowRight size={13} />
                      </>
                    )}
                ...
                {authModalTab === 'login' && (
                  <div className="bg-slate-950/60 border border-slate-850 rounded-2xl p-3 text-[10px] text-slate-400 leading-relaxed font-semibold">
                    <span className="text-indigo-400 uppercase font-bold block text-[8px] tracking-wider mb-0.5">Quick Demo Logins</span>
                    <p className="mb-0.5">• Diner Profile: <b className="text-slate-300">marcus@sterling.co</b> / <span className="font-mono bg-slate-900 border border-slate-800 px-1 rounded">password</span></p>
                    <p>• Chef Owner: <b className="text-slate-300">contact@sakuraomakase.com</b> / <span className="font-mono bg-slate-900 border border-slate-800 px-1 rounded">password</span></p>
                  </div>
                )}
